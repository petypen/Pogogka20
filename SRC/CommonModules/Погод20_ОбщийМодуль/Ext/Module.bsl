...
//  Структура - данные для заполнения виджета 
//
Функция ЗаполнитьЭлементыВиджета(Знач стДанныеПрогноза) Экспорт
	Перем стДанныеВиджета, ШрифтПрогноз, ЦветПрогноз, ШрифтСегодня, ЦветСегодня;
	Перем фсДанные1, фсДанные2, фсЗаголовок;
	
	ШрифтПрогноз = ШрифтыСтиля.ШрифтТекстаИнформационнойПанели;
	ЦветПрогноз  = ЦветаСтиля.ЦветТекстаИнформационнойПанели;	
	ШрифтСегодня = Новый Шрифт("Arial",30,Истина);
	ЦветСегодня  = Новый Цвет(154,154,154);	
	...
	стДанныеВиджета.Вставить("ПОЛЬЗ0_Инфо",                Новый ФорматированнаяСтрока(""));
	// СЕЙЧАС	
	фсДанные1 = Новый ФорматированнаяСтрока(ТемператураКакСтрока(стДанныеПрогноза.мПрогноз[0].чТемпература), ШрифтСегодня, ЦветСегодня);
	фсДанные2 = Новый ФорматированнаяСтрока(Символ(0176), ШрифтСегодня, ЦветСегодня);
