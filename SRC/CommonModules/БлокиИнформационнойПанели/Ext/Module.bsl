...
Процедура ПОЛЬЗ0(стПараметры) Экспорт
	Перем Шрифт, ЦветТекста;
	Перем стЛокация, стДанные;
	...
	Если ТипЗнч(стПараметры.Организация) <> Тип("СправочникСсылка.Организации") ИЛИ стПараметры.Организация = Справочники.Организации.ПустаяСсылка() Тогда
		Возврат;
	КонецЕсли;
	
	стЛокация = Новый Структура();
	Если НЕ Погод20_ОбщийМодуль.ЛокацияОрганизации(стПараметры.Организация, стЛокация) Тогда
		Возврат
	КонецЕсли;
	
	стДанные = Новый Структура();
	Инициализировать_стДанные(СвойстваПОЛЬЗ0(), стДанные);
	Если НЕ Погод20_ОбщийМодуль.ДанныеПогоды(стЛокация, стДанные) Тогда
		Возврат;
	КонецЕсли;
	...
	стДанные = Новый Структура();
	стДанные.Вставить("ПОЛЬЗ0_Заголовок", "ПОГОДКА 2.0");
...
// Процедура - Инициализировать структуру стДанные по свойствам информационного блока
//
// Параметры:
//	сСвойстваБлока - Строка - (IN) Свойства блока, как строка через запятую
//  стДанные - Структура - (IN/OUT) Пустая структура с ключами в соответствии со свойствами блока
//
Процедура Инициализировать_стДанные(Знач сСвойстваБлока, стДанные)
